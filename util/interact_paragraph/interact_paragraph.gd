# NOTE: Taken from https://github.com/Fadi-git/InteractParagraph
@tool
...
@export var exclude_punctuation: bool:
	get:
		return _exclude_punctuation
	set(value):
		_exclude_punctuation = value
...
func _update_paragraph()-> void:
	...
	for lineIdx in paragraph.get_line_count():
		...
		for iGlyph in glyphs.size():
			...
			if is_space or is_punctuation or iGlyph == glyphs.size() - 1:
				var endAdvance := 0.0 if is_space or is_punctuation else advance
				if curWordCharStart != -1:
...
#func _is_puncuation(glyph) -> bool:
	#glyph.
	#pass
